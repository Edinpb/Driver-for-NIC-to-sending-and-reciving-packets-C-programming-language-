...
ssize_t my_read( struct file *file, char *buf, size_t len, loff_t *pos )
{
	...
	count = 22; //22 bytes of payload in ping packets

	// here we try to copy these bytes to the user's buffer 
	if ( copy_to_user( buf, from + 76, count ) ) return -EFAULT; //Payload actually start on 76 byte.
...
static int __init nic_init( void )
{
	...
	create_proc_info_entry( info_tx, 0, NULL, my_get_info_tx );
